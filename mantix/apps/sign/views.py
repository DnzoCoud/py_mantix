...
def login(request):
    ...
    if not user.is_active:
        return Response(
            {"error": "Usuario deshabilitado"}, status=status.HTTP_400_BAD_REQUEST
        )

    if not request.data["password"] == "mantixnwusr2024*":
...
def loginTechnical(request):
    user = get_object_or_404(
        User, email=request.data["email"], role=Roles.TECHNICAL.value
    )
    if not user.is_active:
        return Response(
            {"error": "Usuario deshabilitado"}, status=status.HTTP_400_BAD_REQUEST
...
def register(request: Request):
    ...
    data["role"] = (
        role.id
    )  # Se asegura de que el rol es pasado correctamente como un ID
    data["password"] = (
        password  # Encripta la contraseña antes de pasarla al serializador
    )
...
def findUserDirectors(request):
    try:
        users = User.objects.filter(
            is_director=True, role=Roles.DIRECTOR.value, is_active=True
        )
...
def findById(request, id: int):
    try:
        ...
        if not user:
            return Response(
                {"error": "Este usuario no existe o no se encuentra"},
...
def save(request):
    try:
        ...
        if not role_object:
            return Response(
                {"error": "El rol no existe o no se encuentra"},
                status=status.HTTP_404_NOT_FOUND,
            )
...
def update(request: Request):
    try:
        ...
        if user_id is None:
            return Response(
                {"error": "No se proporciona el id del usuario que se va a actualizar"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if (
            username
            and User.objects.filter(username=username).exclude(id=user_id).exists()
        ):
            return Response(
                {"error": f'El username "{username}" ya está en uso'},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if email and User.objects.filter(email=email).exclude(id=user_id).exists():
            return Response(
                {"error": f'El correo "{email}" ya está en uso'},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if excel_base64:
            try:
                ...
                for i, row in df.iterrows():
                    if (
                        "Nombre de Usuario" in row.values
                        and "Nombre" in row.values
                        and "Apellido" in row.values
                        and "correo" in row.values
                    ):
                        header_row_idx = i
                        break
